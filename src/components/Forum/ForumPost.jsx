...
const ForumPost = ({ id, isLiked, numberOfLikes, numberOfComments, createdBy, title, description }) => {
    ...
    return (
        <Container className='text-align-start'>
            <Row as="h1" style={{ textAlign: "left", color: "#404E5C" }}>{title}</Row>
            <Row>
                <Row>{description}</Row>
                <div className='d-flex justify-content-start'>
                    <div className='p-0 m-0'>{formatLikes(likesCount)}</div>
                    <div className='p-0  ms-2'>

                        {liked ? <FaHeart onClick={handleDislike} style={{ cursor: "pointer" }} /> :
                    ...
                    <div className='ms-2'>Ми се допаѓа</div>
            ...
            <h2 className="mt-4">Коментари</h2>
            {/* Here you could add a component for displaying comments. */}
        </Container>
    );
}
